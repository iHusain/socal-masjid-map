...
class USMasjidMapGenerator:
    ...
    def generate_map(self, title: Optional[str] = None) -> None:
        ...
        if any(
            gdf is None
            for gdf in [self.counties_gdf, self.highways_gdf, self.masjids_gdf]
        ):
        ...
        fig, ax = self.renderer.render_complete_map(
            self.counties_gdf, self.highways_gdf, self.masjids_gdf, title=title
        )
    ...
    def export_map(self, filename: str = "us_masjid_map") -> List[str]:
        ...
        exported_files = self.exporter.export_all_formats(self.renderer.fig, filename)
